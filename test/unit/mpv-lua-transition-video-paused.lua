...
mp.msg.info("[ParadoxFX] Lua script loaded! Preparing to load video: " .. video_file)

function on_start_file()
    mp.msg.info("[ParadoxFX] Loading video and pausing on first frame for 2 seconds...")
    mp.set_property("pause", "yes")
    paused_on_first = true
    mp.add_timeout(2, function()
        mp.msg.info("[ParadoxFX] ...transition now....")
...
function on_eof_reached(name, value)
    if value and not paused_on_last then
        paused_on_last = true
        mp.msg.info("[ParadoxFX] ...paused on last frame (window will stay)...")
        -- Do NOT set pause=yes or reload file, just wait and quit
        mp.add_timeout(8, function()
            mp.msg.info("[ParadoxFX] ...done.")
            mp.commandv("quit")
        end)
    end
end
