...
describe('Real Media Playback Tests', () => {
        ...
        test('should demonstrate seamless full-screen transitions with overlapping processes', async () => {
            ...
            try {                // Step 1: Start first image and leave it running
                ...
                videoProcess = spawn('mpv', [
                    ...
                    '--audio-delay=0',        // No audio delay
                    '--autosync=0',           // Disable auto sync
                    '--mc=0',                 // Disable A/V sync correction
                    '--video-sync=display-resample', // Sync to display
                    '--no-resume-playback',   // Don't resume from saved position
                    '--reset-on-next-file=all', // Reset all settings for next file
                    videoPath
                ], {
                    stdio: 'pipe',
    ...
    test('should demonstrate seamless transitions using VLC (alternative approach)', async () => {
        if (!REAL_PLAYBACK_ENABLED) return;

        console.log('\n🎬 Starting VLC-based seamless transition test...');
        console.log('🔧 This test uses VLC instead of MPV to avoid zoom effects');
        console.log('🎯 Expected sequence: Image₁ → Video (VLC) → Image₂');

        // Timing configuration (slower to address timing issues)
        const TIMING_CONFIG = {
            VIDEO_START_DELAY: 5000,      // Wait 5s after image1 (longer)
            VIDEO_DURATION: 10000,        // Let video play for 10s
            FINAL_DISPLAY_TIME: 4000      // Show final image for 4s
        };
        ...
        try {
            // Step 1: Start first image
            console.log('\n▶️  Step 1: Starting first image (imv-wayland)');
            image1Process = spawn('imv-wayland', ['-f', image1Path], {
                stdio: 'pipe',
                env: { ...process.env }
            });
            console.log('   ✅ First image displayed');

            // Wait before starting video
            console.log(`\n⏱️  Waiting ${TIMING_CONFIG.VIDEO_START_DELAY}ms...`);
            await new Promise(resolve => setTimeout(resolve, TIMING_CONFIG.VIDEO_START_DELAY));

            // Step 2: Stop image and start video with VLC
            console.log('\n▶️  Step 2: Switching to video (VLC - no zoom)');

            if (image1Process && !image1Process.killed) {
                image1Process.kill('SIGTERM');
                console.log('   ⏹️  First image stopped');
            }

            // Wait a moment for clean transition
            await new Promise(resolve => setTimeout(resolve, 200));

            videoProcess = spawn('vlc', [
            ...
            console.log('   ✅ Video started with VLC (should have no zoom effect)');

            // Step 3: Wait for video to complete, then start final image
            console.log(`\n⏱️  Letting video play for ${TIMING_CONFIG.VIDEO_DURATION}ms...`);
            ...
            await new Promise((resolve) => {
                ...
                videoProcess.on('close', (code) => {
                    if (!videoEnded) {
                        videoEnded = true;
                        console.log('   ✅ VLC video completed');
                        resolve();
                    }
                });
                ...
                setTimeout(() => {
                    if (!videoEnded && !videoProcess.killed) {
                        videoProcess.kill('SIGTERM');
                        console.log('   ⏹️  VLC video stopped after timeout');
                        videoEnded = true;
                        resolve();
                    }
                }, TIMING_CONFIG.VIDEO_DURATION);
            });

            // Step 4: Start final image
            console.log('\n▶️  Step 3: Starting final image');

            // Brief pause for clean transition
            await new Promise(resolve => setTimeout(resolve, 300));

            image2Process = spawn('imv-wayland', ['-f', image2Path], {
                stdio: 'pipe',
                env: { ...process.env }
            });

            console.log('   ✅ Final image displayed');

            // Display final image
            console.log(`\n⏱️  Final image display (${TIMING_CONFIG.FINAL_DISPLAY_TIME}ms)`);
            await new Promise(resolve => setTimeout(resolve, TIMING_CONFIG.FINAL_DISPLAY_TIME));

            console.log('\n✨ VLC seamless transition test complete!');
            console.log('🎯 Evaluation checklist:');
            console.log('   ✓ First image appeared cleanly');
            console.log('   ✓ Video started without zoom effect (VLC)');
            console.log('   ✓ Audio played during video (not after)');
            console.log('   ✓ Final image appeared cleanly');

        } finally {
            // Cleanup
            console.log('\n🧹 Cleaning up VLC test processes...');

            [image1Process, videoProcess, image2Process].forEach((proc, idx) => {
                if (proc && !proc.killed) {
                    proc.kill('SIGTERM');
                    console.log(`   ⏹️  Process ${idx + 1} cleaned up`);
                }
            });

            console.log('   ✅ VLC test cleanup complete');
        }
