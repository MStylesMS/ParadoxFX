...
class AudioSetup {
    ...
    /**
     * Internal quick PulseAudio availability check (quiet)
     * @private
     */
    _pulseCheck(timeoutMs = 1000) {
        try {
            execSync('pactl info', { timeout: timeoutMs, stdio: 'ignore' });
            return true;
        } catch (e) {
            return false;
        }
    ...
    /**
     * Wait for PulseAudio to become available up to maxWaitMs.
     * Polls quietly every intervalMs to avoid log spam.
     * Returns true if PulseAudio became available, false if timed out.
     * @param {number} maxWaitMs
     * @param {number} intervalMs
     */
    async waitForPulseAudio(maxWaitMs = 6000, intervalMs = 500) {
        if (this._pulseCheck()) {
            return true; // Already up
        }
        const start = Date.now();
        this.logger.info(`Waiting for PulseAudio (up to ${maxWaitMs}ms)...`);
        while (Date.now() - start < maxWaitMs) {
            if (this._pulseCheck()) {
                const waited = Date.now() - start;
                this.logger.info(`PulseAudio became available after ${waited}ms`);
                return true;
            }
            await new Promise(r => setTimeout(r, intervalMs));
        }
        this.logger.warn('PulseAudio not available after wait period');
        return false;
    }
    ...
    /**
     * Activate slave sinks before creating combined sink
     * @private
     */
    async _activateSlaveSinks(slaves) {
        this.logger.debug('Activating slave sinks...');
        
        for (const slaveName of slaves) {
            try {
                // Play a very brief silent sample to wake up the sink
                // Using timeout to ensure it doesn't hang
                const command = `timeout 0.5 paplay --device=${slaveName} /dev/zero 2>/dev/null || true`;
                exec(command, { timeout: 1000 }, (error, stdout, stderr) => {
                    // Ignore errors - this is just to wake up the sink
                });
                
                // Small delay between activations
                await new Promise(resolve => setTimeout(resolve, 100));
            } catch (error) {
                this.logger.debug(`Failed to activate sink ${slaveName}: ${error.message}`);
                // Continue anyway - some sinks might already be active
            }
        }
        
        // Give PulseAudio a moment to register the state changes
    ...
    async _createCombinedSink(sinkName, description, slaves) {
        // Activate slave sinks first to avoid "Module initialization failed"
        await this._activateSlaveSinks(slaves);
