...
class WindowManager {
    ...
    async waitForWindowByClass(className, timeoutMs = 10000) {
        ...
        while (Date.now() - start < timeoutMs) {
            try {
                // Try xdotool search first (returns decimal ids)
                let out = execSync(`xdotool search --class ${className} | tail -n1`, { env: this.env }).toString().trim();
                if (out) return this._normalizeWindowId(out);
                // Fallback to wmctrl parsing (wmctrl uses hex ids)
                out = execSync('wmctrl -lx', { env: this.env }).toString().split('\n').filter(l => new RegExp(className, 'i').test(l)).slice(-1)[0] || '';
                if (out) {
                    const m = out.match(/^(0x[0-9a-fA-F]+)/);
                    if (m) return this._normalizeWindowId(m[1]);
                }
            } catch (_) {}
    ...
    findChromiumWindowId(customClass = 'ParadoxBrowser') {
        ...
        try {
            const out = execSync(`xdotool search --class ${customClass} | tail -n1`, { env: this.env }).toString().trim();
            if (out) return this._normalizeWindowId(out.split('\n').slice(-1)[0].trim());
        } catch {}
        
        // Fallback to common chromium classes
        for (const cls of ['chromium-browser', 'chromium', 'Chromium']) {
            try {
                const out = execSync(`xdotool search --class ${cls} | tail -n1`, { env: this.env }).toString().trim();
                if (out) return this._normalizeWindowId(out.split('\n').slice(-1)[0].trim());
            } catch {}
        }
    ...
    getWindowIdByNameExact(name) {
        try {
            const out = execSync(`xdotool search --name '^${name}$' | head -n1`, { env: this.env }).toString().trim();
            return out ? this._normalizeWindowId(out) : null;
        } catch { 
            return null; 
        }
    }
    ...
    isWindowActive(winId) {
        try {
            ...
            const active = this._normalizeWindowId(out);
            const wanted = this._normalizeWindowId(winId);
            return active && wanted && active === wanted;
        } catch {
            return false;
        }
    }
     ...
     * Returns array of window IDs (decimal strings)
     * Phase 1 implementation for PR_BROWSER_WINDOW_REFRESH
     */
    async searchByClass(className) {
        const windows = [];
        try {
            const out = execSync(`xdotool search --class ${className}`, { env: this.env }).toString().trim();
            if (out) {
                // xdotool returns multiple IDs, one per line
                const ids = out.split('\n').map(id => this._normalizeWindowId(id)).filter(id => id);
                windows.push(...ids);
            }
        } catch (error) {
            this.logger.debug(`xdotool search --class ${className} failed: ${error.message}`);
        }
        return windows;
    }

    /**
     * Get window-to-PID mapping using wmctrl -lp
     * Returns object: {windowId: pid, ...}
     * Phase 1 implementation for PR_BROWSER_WINDOW_REFRESH
     */
    async getWindowPidMap() {
        const map = {};
        try {
            const out = execSync('wmctrl -lp', { env: this.env }).toString();
            // Format: 0x03a00007  0  12345  hostname  Window Title
            const lines = out.split('\n').filter(line => line.trim());
            
            for (const line of lines) {
                const parts = line.trim().split(/\s+/);
                if (parts.length >= 3) {
                    const windowIdHex = parts[0];
                    const pid = parseInt(parts[2], 10);
                    
                    if (windowIdHex && !isNaN(pid)) {
                        const normalizedId = this._normalizeWindowId(windowIdHex);
                        if (normalizedId) {
                            map[normalizedId] = pid;
                        }
                    }
                }
            }
        } catch (error) {
            this.logger.debug(`wmctrl -lp failed: ${error.message}`);
        }
        return map;
    }

    /**
     * Activate window using wmctrl -i -a
     * Alternative to xdotool windowactivate
     * Phase 1 implementation for PR_BROWSER_WINDOW_REFRESH
     */
    async wmctrlActivate(windowId) {
        try {
            execSync(`wmctrl -i -a ${windowId}`, { env: this.env });
            return true;
        } catch (error) {
            this.logger.debug(`wmctrl -i -a ${windowId} failed: ${error.message}`);
            return false;
        }
    }

    /**
     * Aggressive fallback: unmap, raise, and focus window
     * Used when standard activation fails
     * Phase 1 implementation for PR_BROWSER_WINDOW_REFRESH
     */
    async unmapRaiseFocus(windowId) {
        try {
            // Try to unmap (make visible)
            try {
                execSync(`xdotool windowmap ${windowId}`, { env: this.env });
            } catch (e) {
                this.logger.debug(`xdotool windowmap failed: ${e.message}`);
            }
            
            // Raise to top
            try {
                execSync(`xdotool windowraise ${windowId}`, { env: this.env });
            } catch (e) {
                this.logger.debug(`xdotool windowraise failed: ${e.message}`);
            }
            
            // Focus the window
            try {
                execSync(`xdotool windowfocus ${windowId}`, { env: this.env });
            } catch (e) {
                this.logger.debug(`xdotool windowfocus failed: ${e.message}`);
            }
            
            return true;
        } catch (error) {
            this.logger.error(`unmapRaiseFocus failed: ${error.message}`);
            return false;
        }
    }

    /**
     * Normalize window id string to decimal string form.
     * Accepts formats like '0x3a00007' or '37865927' and returns decimal string.
     */
    _normalizeWindowId(raw) {
        ...
        try {
            ...
            // some xdotool outputs may include whitespace or multiple ids
            const first = raw.split('\n')[0].trim();
            // if numeric decimal
            if (/^\d+$/.test(first)) return first;
            // try parse as hex without 0x
            if (/^[0-9a-fA-F]+$/.test(first)) return String(parseInt(first, 16));
            return null;
        } catch (e) {
            return null;
        }
    ...
    /**
     * Pick available Chromium binary using OS detection
     */
    _pickChromiumBinary() {
        // Use OS detection to get the appropriate browser command
        try {
            const { getOSDetection } = require('./os-detection');
            const osInfo = getOSDetection();
            const browserCmd = osInfo.getBrowserCommand();
            
            // Verify the detected command exists
            if (this._binExists(browserCmd)) {
                return browserCmd;
            }
        } catch (error) {
            // Fall through to manual detection if OS detection fails
        }
        
        // Fallback: try known candidates manually
        const candidates = ['chromium', 'chromium-browser', 'google-chrome'];
        for (const c of candidates) {
            if (this._binExists(c)) return c;
        }
        throw new Error('Chromium binary not found (tried chromium, chromium-browser, google-chrome). Install chromium or chromium-browser.');
    }
