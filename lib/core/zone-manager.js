...
class ZoneManager {
    ...
    async _handleZoneCommand(zoneName, zone, message) {
        ...
        try {
            ...
            // Log the command at INFO level for better visibility
            const commandName = command.Command || command.command;
            this.logger.info(`Zone ${zoneName} received valid command: ${commandName}`);
