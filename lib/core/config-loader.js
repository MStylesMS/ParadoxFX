    ...
    _processDeviceConfig(deviceName, config) {
        ...
        switch (deviceType) {
            case 'screen':
                ...
                // Support new target_monitor with backward compatibility
                console.log(`🔧 DEBUG: config.target_monitor = ${config.target_monitor}, config.xinerama_screen = ${config.xinerama_screen}`);
                
                if (config.target_monitor !== undefined || config.TARGET_MONITOR !== undefined) {
                    processed.targetMonitor = parseInt(config.target_monitor || config.TARGET_MONITOR) || 0;
                    console.log(`🔧 DEBUG: Using target_monitor: ${processed.targetMonitor}`);
                } else {
                    // Legacy support for xinerama_screen
                    processed.targetMonitor = parseInt(config.xinerama_screen || config.XINERAMA_SCREEN) || 0;
                    console.log(`🔧 DEBUG: Fallback to xinerama_screen: ${processed.targetMonitor}`);
                    if (config.xinerama_screen !== undefined) {
                        console.warn(`⚠️  Legacy config detected: 'xinerama_screen' is deprecated. Please use 'target_monitor' instead.`);
                    }
                }
