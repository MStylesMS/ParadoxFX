...
class MpvProfileManager {
    ...
    buildMpvArgs(profileName, audioDevice, xineramaScreen) {
        ...
        let mpvArgs = [
            ...
            // Display targeting (use fs-screen for fullscreen targeting)
            `--fs-screen=${xineramaScreen}`,
            ...
            '--no-terminal',
            '--no-osd-bar',
            // '--really-quiet',  // Temporarily removed for debugging
            '--idle=yes',
            '--keep-open=yes',
            '--keep-open-pause=no'  // Prevent automatic pausing when keep-open is active
        ];
...
class MpvZoneManager {
    constructor(config) {
        ...
        this.mpvProfile = config.mpvVideoProfile || 'generic-minimal'; // Use profile from config with fallback
    }
    ...
    async initialize() {
        ...
        try {
            // Load profiles - no autodetection, use config setting
            await this.profileManager.loadProfiles();
            this.logger.info(`Using configured MPV profile: ${this.mpvProfile}`);
    ...
    async _startMpvProcess() {
        ...
        this.mpvProcess.stderr.on('data', (data) => {
            this.logger.info('MPV stderr:', data.toString().trim());
        });
    }
