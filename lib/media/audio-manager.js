...
class AudioManager {
    ...
    async playBackgroundMusic(filePath, volume = null, loop = false) {
        ...
        // Publish start event (flattened) via zone if available
        if (this.zone) {
            try {
                    ...
                    const monitor = new PlaybackMonitor({
                        ...
                        onComplete: () => {
                            if (!loop) {
                                ...
                                // Loop iteration ended; emit loop event (done false) and restart monitor for next cycle
                                this.zone.publishMessage('events', {
                                    command: 'playBackground',
                                    file: path.basename(filePath),
                                    done: false,
                                ...
                                // Restart monitor for next loop iteration
                                setTimeout(() => {
                                    if (!this.isShuttingDown) monitor.start();
                                }, 10);
                            }
                        },
                        onInterrupt: () => {
                            ...
                            this.zone.publishMessage('events', {
                                ...
                                message: 'File play interrupted'
                            });
                        }
                    });
                    // Store monitor reference so we can interrupt on stop
                    this._backgroundMonitor = monitor;
                    monitor.start();
                }
            }
