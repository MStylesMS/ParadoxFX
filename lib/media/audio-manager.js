...
class AudioManager {
    constructor(config) {
        ...
        // Sanitize zoneId for filesystem socket names (replace ':' or '/' with '-')
        const sanitizedZoneId = zoneId.replace(/[:\/]/g, '-');
        this.backgroundMusicSocket = `/tmp/pfx-background-music-${sanitizedZoneId}.sock`;
        this.speechSocket = `/tmp/pfx-speech-${sanitizedZoneId}.sock`;
    ...
    async checkAndRestartProcesses() {
        ...
        // For speech system, we don't maintain a persistent process, so less to check
    ...
    async playSpeech(filePath, volume = null) {
        if (!fs.existsSync(filePath)) {
            this.logger.warn(`Speech file not found: ${filePath}`);
            return { success: false, error: `Speech file not found: ${filePath}` };
        }
    ...
    async _processSpeechQueue() {
        ...
        while (this.speechQueue.length > 0) {
            ...
            try {
                ...
                // Wait for speech to complete (listen for end-file event)
                await this._monitorEndFile(this.speechSocket);
    ...
    /**
     * Monitor MPV end-file events
     * @private
     */
    _monitorEndFile(socketPath) {
        return new Promise((resolve, reject) => {
            const client = net.createConnection(socketPath, () => {});
            let buffer = '';
            const timeout = setTimeout(() => {
                client.destroy();
                reject(new Error('End-file monitoring timed out'));
            }, 30000);
            client.on('data', (chunk) => {
                buffer += chunk.toString();
                let newlineIndex;
            ...
            client.on('error', (err) => {
                clearTimeout(timeout);
                reject(new Error(`End-file monitoring error: ${err.message}`));
            });
        });
    }
