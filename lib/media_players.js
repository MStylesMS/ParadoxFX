...
class AudioPlayer {
    ...
    playAudio(file, volume = 1.0, type = 'plain') {
        ...
        if (this.activePlayers.length >= this.maxPolyphony) {
            console.error(`[PxFx] Max polyphony reached, cannot play: ${file}`);
            return;
        }
        ...
        player.on('exit', () => {
            console.log(`[PxFx] Audio playback finished: ${file}`);
            this.activePlayers = this.activePlayers.filter(p => p !== player);
        });

        player.on('error', (err) => {
            console.error(`[PxFx] Error during audio playback: ${file}`, err);
        });

        console.log(`[PxFx] Playing audio: ${file} at volume ${adjustedVolume}`);
    }

    stopAllAudio() {
        console.log('[PxFx] Stopping all audio playback');
