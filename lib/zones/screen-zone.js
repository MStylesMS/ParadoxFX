...
class ScreenZone extends BaseZone {
    constructor(config, mqttClient, zoneManager) {
        ...
        // Replace direct-play handlers with our enqueueing
        this.registerCommand('playVideo', this._enqueueVideoCommand.bind(this));
        this.registerCommand('setImage',  this._enqueueVideoCommand.bind(this));
    ...
    async _processVideoQueue() {
        ...
        // Allow processing during video playback - queue system should work concurrently
    ...
    _handleMediaEnd() {
        ...
        // Process next item in queue after a brief delay to ensure state is settled
        if (this.videoQueue.length > 0) {
