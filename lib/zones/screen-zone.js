...
class ScreenZone extends BaseZone {
    ...
    async _playVideo(command) {
        ...
        const { volume, adjustVolume, channel, ducking } = command;
        ...
        this.publishEvent({
            ...
            duck_id: duckId,
            pre_duck: resolvedVideo ? resolvedVideo.preDuck : undefined,
            ducked: resolvedVideo ? resolvedVideo.ducked : undefined
        });
        if (resolvedVideo && resolvedVideo.warnings && resolvedVideo.warnings.length) {
            this.publishCommandOutcome({ command: 'playVideo', outcome: 'warning', parameters: { file: videoPath, volume: resolvedVideo.final, warnings: resolvedVideo.warnings.map(w => w.code) }, warning_type: 'volume_resolution_warning', message: 'Video playback started with volume resolution warnings' });
        }
    ...
    async _stopVideo(fadeTime = 0) {
        ...
        if (fadeTime > 0 && this.mpvZoneManager && typeof this.mpvZoneManager.fadeVolume === 'function') {
            // If zone manager exposes a fadeVolume helper, use it
            try {
                await this.mpvZoneManager.fadeVolume(0, fadeTime * 1000, async () => {
                    await this.mpvZoneManager.stop();
                });
            } catch (e) {
                this.logger.debug('Video fadeVolume failed or unsupported, falling back to manual fade/stop:', e.message);
                await this.mpvZoneManager.stop();
            }
        } else if (fadeTime > 0 && this.mpvZoneManager) {
            // Manual fade loop using set_property volume if supported
            let supported = true;
            let startVol = 100;
            try {
                const resp = await this.mpvZoneManager.sendCommand(['get_property', 'volume']);
                if (resp && resp.data !== undefined) startVol = resp.data;
            ...
            if (supported) {
                ...
                for (let i = 1; i <= steps; i++) {
                    ...
                    try { await this.mpvZoneManager.sendCommand(['set_property', 'volume', v]); } catch (_) { /* ignore */ }
        ...
        this.publishStatus();
        this.publishEvent({ video_stopped: true, fade_time: fadeTime });

        // PR-VOLUME Phase 3: remove lifecycle trigger for video
        if (this.currentState.currentVideoLifecycleId) {
            if (this.duckLifecycle) {
                this.duckLifecycle.removeTrigger(this.currentState.currentVideoLifecycleId);
                this.logger.debug(`DuckLifecycle: removed video trigger ${this.currentState.currentVideoLifecycleId}`);
            }
            this.currentState.currentVideoLifecycleId = null;
        }
    }
    ...
    async _playBackgroundMusic(audioPath, params = {}) {
        ...
        if (!result.success) {
            this.publishCommandOutcome({ command: 'playBackground', outcome: 'failed', parameters: { file: audioPath, volume: targetVolume }, error_type: 'play_error', error_message: result.error, message: `Failed to start background music: ${result.error}` });
            return false;
        }
        ...
        this.publishStatus();
        this.publishEvent({ background_music_started: true, file: audioPath, volume: targetVolume, pre_duck: resolved.preDuck, ducked: resolved.ducked, adjust_volume: (params && params.adjustVolume) || 0 });
        if (resolved.warnings && resolved.warnings.length) {
            this.publishCommandOutcome({ command: 'playBackground', outcome: 'warning', parameters: { file: audioPath, volume: targetVolume, warnings: resolved.warnings.map(w => w.code) }, warning_type: 'volume_resolution_warning', message: 'Background playback started with volume resolution warnings' });
        }
        return true;
    }
    ...
    async _playSpeech(audioPath, volume, ducking, opts = {}) {
        ...
        this.publishStatus();
        this.publishEvent({ speech_started: audioPath, volume: targetVolume, ducking_applied: duckingLevel, duck_id: duckId, adjust_volume: adjustVolume || 0 });
    ...
    async _recomputeBackgroundAfterDuckChange() {
        ...
        try {
            ...
            this.publishEvent({ background_volume_recomputed: true, volume: resolved.final, pre_duck: resolved.preDuck, ducked: resolved.ducked });
            this.publishStatus();
        } catch (e) {
    ...
    async _processVideoQueue() {
        ...
        try {
            ...
            // Clean up any incomplete ducking operations (Phase 8 lifecycle based)
            if (this.currentState.currentVideoDuckId) {
                if (this.duckLifecycle) {
                    this.duckLifecycle.removeTrigger(this.currentState.currentVideoDuckId);
                    try { await this._recomputeBackgroundAfterDuckChange(); } catch (_) { /* swallow */ }
                }
                this.currentState.currentVideoDuckId = null;
            }
    ...
    _handleMediaEnd() {
        ...
        if (this.currentState.currentVideoDuckId) {
            this.duckLifecycle.removeTrigger(this.currentState.currentVideoDuckId);
            this.currentState.currentVideoDuckId = null;
            this._recomputeBackgroundAfterDuckChange();
            this.logger.debug('Removed video duck lifecycle trigger on media end');
        }

        // PR-VOLUME Phase 3: remove lifecycle trigger for video (EOF path)
        if (this.currentState.currentVideoLifecycleId) {
            if (this.duckLifecycle) {
                this.duckLifecycle.removeTrigger(this.currentState.currentVideoLifecycleId);
                this.logger.debug(`DuckLifecycle: removed video trigger (EOF) ${this.currentState.currentVideoLifecycleId}`);
            }
            this.currentState.currentVideoLifecycleId = null;
        }
        ...
        // Publish completion before possibly starting next item
        if (completedVideoFile) {
            this.publishEvent({ video_completed: completedVideoFile });
        }

        // If queue has more items, start next
        if (this.videoQueue.length > 0) {
            const nextItem = this.videoQueue.shift();
            if (nextItem.type === 'video') {
                this._playVideo(nextItem.path, nextItem.loop, nextItem.mute, nextItem.volume, nextItem.ducking, nextItem.fadeIn, nextItem.fadeOut);
            } else if (nextItem.type === 'image') {
                this._showImage(nextItem.path, nextItem.duration, nextItem.lock);
            }
            this.publishStatus();
            return;
        }

        // If we were in a one-shot playback (queue empty) just publish idle status
        this.publishStatus();
    }
