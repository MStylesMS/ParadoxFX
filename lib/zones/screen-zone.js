...
class ScreenZone extends BaseZone {
    ...
    async _playVideo(command) {
        ...
        // PR-VOLUME Phase 3: Always register a unified lifecycle trigger for video playback
        if (isVideo) {
            const lifecycleId = duckId || `videoTrigger-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
            this.currentState.currentVideoLifecycleId = lifecycleId;
            if (this.duckLifecycle) {
                this.duckLifecycle.addTrigger(lifecycleId, 'video');
                this.logger.debug(`DuckLifecycle: added video trigger ${lifecycleId}`);
            }
        }
    ...
    async _playSpeech(audioPath, volume, ducking) {
        ...
        // Generate unique duck ID for this speech (used for lifecycle + legacy ducking)
        const duckId = `speech-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

        // Legacy absolute duck application (will be removed in later phase)
        if (duckingLevel < 0) {
            this._applyDucking(duckId, duckingLevel);
        }
        // New lifecycle trigger
        if (this.duckLifecycle) {
            this.duckLifecycle.addTrigger(duckId, 'speech');
            this.logger.debug(`DuckLifecycle: added speech trigger ${duckId}`);
        }
        ...
        try {
            ...
            // Cleanup lifecycle/duck when playback promise resolves (speech item done)
            if (duckingLevel < 0) {
                this._removeDucking(duckId);
            }
            if (this.duckLifecycle) {
                this.duckLifecycle.removeTrigger(duckId);
                this.logger.debug(`DuckLifecycle: removed speech trigger ${duckId}`);
            }
            this.mpvInstances.speech.status = 'idle';
            this.publishStatus();
            if (playbackError) {
                this.publishEvent({ speech_error: audioPath, error: playbackError.message || String(playbackError) });
            }
        }

        this.logger.info(`Speech playback initiated: ${audioPath} at volume ${targetVolume}`);
    }
