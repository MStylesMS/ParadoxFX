...
class ScreenZone extends BaseZone {
        ...
        try {
            switch (command.command) {
                ...
                case 'stopBackground':
                    ...
                    break;
                case 'playSpeech':
                    await this._playSpeech(command.audio, command.volume, command.ducking);
                    break;
                case 'pauseSpeech':
    ...
    getSupportedCommands() {
        return [
            ...
            'playSpeech', 'pauseSpeech', 'resumeSpeech', 'skipSpeech', 'speechQueue', 'playAudioFx', 'playSoundEffect', 'stopAudio',
            'sleepScreen', 'wakeScreen', 'recoverScreens',
            'reboot', 'shutdown', 'killPfx',
            'setVolume', 'getStatus', 'stopAll', 'pauseAll', 'resumeAll',
    ...
    async _playVideo(command) {
        const { video: videoPath, volumeAdjust, channel, ducking } = command;
        if (!videoPath) {
    ...
    async _enqueueVideoCommand(command) {
        ...
        // Ignore duplicate playVideo if the same video is already playing
    ...
    async _processVideoQueue() {
        ...
        // Only process when player is idle (not currently playing or paused)
     ...
     */
    _handleMediaEnd() {
        this.logger.debug('Received end-file event. Queue length: ' + this.videoQueue.length);
    ...
    /**
     * Enable browser: Launch Chromium and position on target display
     */
    async _enableBrowser(url = 'http://localhost/clock/', focus = true) {
        if (this.browserManager.enabled) {
            this.logger.warn('Browser already enabled');
            return;
        }

        this.logger.info(`Enabling browser with URL: ${url}`);

        // Clean up any existing browser artifacts
        this.windowManager.safeRemoveDir(this.browserManager.profilePath);

        // Determine target display
        const targetDisplay = this.windowManager.pickTargetDisplay(true);
        this.logger.debug(`Browser target display: ${targetDisplay.name} at ${targetDisplay.width}x${targetDisplay.height}+${targetDisplay.x}+${targetDisplay.y}`);
        ...
        this.logger.info('Browser enabled successfully');
    }
    ...
    async _disableBrowser() {
        ...
        this.logger.info('Disabling browser');
    ...
    /**
     * Show browser: Bring browser to front with optional fade effect
     */
    async _showBrowser(effect = 'fade') {
        ...
        this.logger.info(`Showing browser with effect: ${effect}`);
        ...
        // Switch focus to browser using Option 6 technique
        this.windowManager.activateWindow(this.browserManager.windowId);
        ...
        this.logger.info('Browser shown successfully');
    }

    /**
     * Hide browser: Return focus to MPV with optional fade effect
     */
    async _hideBrowser(effect = 'fade') {
        ...
        this.logger.info(`Hiding browser with effect: ${effect}`);
    ...
    async _setBrowserUrl(url) {
        ...
        if (this.browserManager.enabled) {
            ...
            await this._disableBrowser();
            await this._enableBrowser(url, wasFocused);
        } else {
    ...
    async _setBrowserKeepAlive(enabled) {
        ...
        // TODO: Implement process monitoring if keepAlive is enabled
        if (enabled) {
            this.logger.debug('Browser keep-alive monitoring not yet implemented');
        }
    }
    ...
    /**
     * Switch to MPV: Hide browser and show video content with optional fade
     */
    async _switchToMpv(effect = 'fade') {
        this.logger.info(`Switching to MPV with effect: ${effect}`);

        // If browser is enabled and focused, hide it first
        if (this.browserManager.enabled && this.currentState.focus === 'chromium') {
            await this._hideBrowser(effect);
        } else {
    ...
    /**
     * Switch to browser: Hide MPV content and show browser with optional fade
     */
    async _switchToBrowser(effect = 'fade') {
        if (!this.browserManager.enabled) {
            throw new Error('Browser not enabled. Call enableBrowser first.');
        }

        this.logger.info(`Switching to browser with effect: ${effect}`);
        ...
        if (this.currentState.focus === 'mpv') {
            ...
            if (effect === 'fade') {
                await this._publishClockCommand({ command: 'fadeOut' });
                await this._delay(1000); // Allow fade to progress
            }

            // Switch to browser
            this.windowManager.activateWindow(this.browserManager.windowId);

            // Optional fade-in browser content
            if (effect === 'fade') {
                await this._delay(500); // Brief pause
                await this._publishClockCommand({ command: 'fadeIn' });
            }
            ...
            await this._showBrowser(effect);
        }
     ...
     */
    async _toggleMpvBrowser(effect = 'fade') {
        this.logger.info('Toggling between MPV and browser');

        if (this.currentState.focus === 'chromium') {
            await this._switchToMpv(effect);
        } else if (this.currentState.focus === 'mpv') {
            if (this.browserManager.enabled) {
                await this._switchToBrowser(effect);
            } else {
