...
class ScreenZone extends BaseZone {
    ...
    async initialize() {
        ...
        try {
            ...
            this.logger.debug('Cleaning up stale MPV socket files...');
            try {
                const { exec } = require('child_process');
                const util = require('util');
                const execAsync = util.promisify(exec);
                await execAsync('rm -f /tmp/mpv-*.sock /tmp/pfx-*.sock');
                this.logger.debug('Socket cleanup completed');
            } catch (err) {
                this.logger.debug('Socket cleanup encountered error (non-fatal):', err.message);
            }
    ...
    async _playVideo(command) {
        ...
        this._videoPlaybackTracker = new VideoPlaybackTracker({
            ...
            onNaturalEnd: () => {
                this.logger.info('VIDEO_TRACKER natural end fired');
    ...
    async _handleLoopRestart() {
        ...
        this.logger.info(`Loop iteration ${this.loopState.loopIterations} for ${videoPath}`);
        ...
        try {
            ...
            this._videoPlaybackTracker = new VideoPlaybackTracker({
                ...
                onNaturalEnd: () => {
                    this.logger.info('VIDEO_TRACKER natural end fired (loop iteration)');
                    if (this.loopState.isLooping) {
                        this._handleLoopRestart().catch(err => {
                            this.logger.error('Loop restart failed:', err);
                            this.loopState.isLooping = false;
                            this.loopState.isRestarting = false; // Clear restart flag on error
                            this._completeCurrentVideo('error', { error: err.message });
                        });
                    } else {
                        this._completeCurrentVideo('natural_end');
            ...
            // Note: VideoPlaybackTracker handles EOF detection
            // Legacy _setupVideoEof removed to prevent race conditions
    ...
    async _handlePlayVideoQueue(mediaPath, mediaType, command) {
        if (mediaType === 'image') {
            ...
            // Note: VideoPlaybackTracker (created in _playVideo) handles EOF detection
            // No need for legacy _setupVideoEof which was causing race conditions
        }
    }
    ...
    async _enableBrowser(url = 'http://localhost/clock/') {
        ...
        // Get OS-specific window detection configuration
        ...
        // Wait for browser window to appear (OS-specific timeout)
        ...
        if (!this.browserManager.windowId) {
            // Fallback detection with OS-specific retries
            for (let attempt = 1; attempt <= windowConfig.maxRetries && !this.browserManager.windowId; attempt++) {
                this.logger.debug(`enableBrowser: fallback attempt ${attempt}/${windowConfig.maxRetries} to find Chromium window`);
                this.browserManager.windowId = this.windowManager.findChromiumWindowId(this.browserManager.className);
                if (!this.browserManager.windowId) await new Promise(r => setTimeout(r, windowConfig.retryDelay));
            }
    ...
    async _showBrowser() {
        ...
        const targetPid = this.browserManager.process?.pid;
        this.logger.info(`Showing browser (PID: ${targetPid || 'unknown'})`);

        // Step 1: Refresh candidate Chromium windows by class
        const candidates = await this._findChromiumWindowsByClass();
        if (candidates.length === 0) {
            this.logger.error('No browser windows found by class');
            await this._collectWindowDiagnostics();
            throw new Error('Browser window not found');
        }
        this.logger.debug(`Found ${candidates.length} browser window candidate(s): ${candidates.join(', ')}`);

        // Step 2: Match window by PID (if available)
        let targetWindow = null;
        if (targetPid) {
            targetWindow = await this._matchWindowByPid(candidates, targetPid);
            if (targetWindow) {
                this.logger.info(`Matched browser window ${targetWindow} to PID ${targetPid}`);
            } else {
                this.logger.warn(`No window matched browser PID ${targetPid}, will try all candidates`);
            }
        }

        // If PID-matching failed or no PID available, use first candidate
        if (!targetWindow && candidates.length > 0) {
            targetWindow = candidates[0];
            this.logger.debug(`Using first candidate window: ${targetWindow}`);
        }

        // Normalize the target window ID
        const normalizedTarget = this._normalizeWindowId(targetWindow);

        // Get MPV window for z-order management
        let mpvWindow = null;
        try {
            mpvWindow = this.windowManager.getWindowIdByNameExact('ParadoxMPV');
            if (mpvWindow) {
                // Try to unset 'above' on MPV to avoid ontop conflicts
                try {
                    this.windowManager.removeWindowState(mpvWindow, 'above');
                    this.logger.debug(`Removed 'above' state from MPV window ${mpvWindow}`);
                } catch (errRem) {
                    this.logger.debug(`Failed to remove 'above' from MPV: ${errRem.message}`);
                }
            }
        } catch (err) {
            this.logger.debug(`MPV window lookup failed: ${err.message}`);
        }

        // Set browser to 'above' to ensure it appears over MPV
        try {
            this.windowManager.addWindowState(normalizedTarget, 'above');
            this.logger.debug(`Added 'above' state to browser window ${normalizedTarget}`);
        } catch (errAdd) {
            this.logger.debug(`Failed to add 'above' to browser: ${errAdd.message}`);
        }

        // Step 3: Try xdotool activateWindow up to 3 times
        let becameActive = false;
        for (let attempt = 1; attempt <= 3 && !becameActive; attempt++) {
            try {
                const t0 = Date.now();
                this.windowManager.activateWindow(normalizedTarget);
                this.logger.debug(`activateWindow attempt ${attempt} took ${Date.now() - t0}ms`);
                
                // Step 4: Check if window actually became active
                await new Promise(r => setTimeout(r, attempt * 100));
                becameActive = this.windowManager.isWindowActive(normalizedTarget);
                
                if (becameActive) {
                    this.logger.info(`Browser became active on xdotool attempt ${attempt}`);
                    break;
                }
            } catch (err) {
                this.logger.debug(`activateWindow attempt ${attempt} failed: ${err.message}`);
            }
        }

        // Step 5: Try wmctrl activate if xdotool failed
        if (!becameActive) {
            this.logger.warn('xdotool activation failed, trying wmctrl fallback');
            try {
                const t0 = Date.now();
                const success = await this.windowManager.wmctrlActivate(normalizedTarget);
                this.logger.debug(`wmctrlActivate took ${Date.now() - t0}ms, success=${success}`);
                
                await new Promise(r => setTimeout(r, 150));
                becameActive = this.windowManager.isWindowActive(normalizedTarget);
                
                if (becameActive) {
                    this.logger.info('Browser became active via wmctrl fallback');
                }
            } catch (err) {
                this.logger.debug(`wmctrl fallback failed: ${err.message}`);
            }
        }

        // Step 6: Aggressive fallback - try unmapRaiseFocus on all candidates
        if (!becameActive) {
            this.logger.warn('Standard activation failed, trying aggressive fallback on all candidates');
            
            // If we had a PID-matched window, try it first
            const orderedCandidates = targetWindow && targetPid 
                ? [targetWindow, ...candidates.filter(w => w !== targetWindow)]
                : candidates;

            for (const candidateWin of orderedCandidates) {
                const normalized = this._normalizeWindowId(candidateWin);
                this.logger.debug(`Aggressive fallback: trying window ${normalized}`);
                
                try {
                    await this.windowManager.unmapRaiseFocus(normalized);
                    await new Promise(r => setTimeout(r, 150));
                    
                    if (this.windowManager.isWindowActive(normalized)) {
                        becameActive = true;
                        targetWindow = candidateWin;
                        this.logger.info(`Browser became active via aggressive fallback: ${normalized}`);
                        break;
                    }
                } catch (err) {
                    this.logger.debug(`Aggressive fallback failed for ${normalized}: ${err.message}`);
                }
            }
        }

        // Step 7: If all attempts failed, collect diagnostics
        if (!becameActive) {
            this.logger.error('All browser activation attempts failed');
            await this._collectWindowDiagnostics();
        } else {
            // Success - update stored window ID
            this.browserManager.windowId = this._normalizeWindowId(targetWindow);
            this.logger.info(`Browser shown successfully (${Date.now() - tsStart}ms, window: ${this.browserManager.windowId})`);
        }
    ...
    /**
     * Helper: Find Chromium windows by class name
     * @returns {Array<string>} Array of window IDs
     */
    async _findChromiumWindowsByClass() {
        try {
            const windows = await this.windowManager.searchByClass(this.browserManager.className);
            return windows || [];
        } catch (err) {
            this.logger.warn(`Failed to find Chromium windows by class: ${err.message}`);
            return [];
        }
    }

    /**
     * Helper: Match window to browser PID
     * @param {Array<string>} candidates - Array of window IDs
     * @param {number} targetPid - Browser process PID
     * @returns {string|null} Matched window ID or null
     */
    async _matchWindowByPid(candidates, targetPid) {
        try {
            const pidMap = await this.windowManager.getWindowPidMap();
            if (!pidMap) return null;

            for (const candidateWin of candidates) {
                const normalized = this._normalizeWindowId(candidateWin);
                if (pidMap[normalized] === targetPid) {
                    return candidateWin;
                }
            }
        } catch (err) {
            this.logger.warn(`Failed to match window by PID: ${err.message}`);
        }
        return null;
    }

    /**
     * Helper: Normalize window ID to decimal format
     * @param {string} windowId - Window ID in hex (0xABCD) or decimal format
     * @returns {string} Normalized decimal window ID
     */
    _normalizeWindowId(windowId) {
        if (!windowId) return windowId;
        const str = String(windowId).trim();
        if (str.startsWith('0x') || str.startsWith('0X')) {
            return String(parseInt(str, 16));
        }
        return str;
    }

    /**
     * Helper: Collect window diagnostics for troubleshooting
     */
    async _collectWindowDiagnostics() {
        this.logger.debug('=== Window Diagnostics ===');
        
        // Get all windows with geometry
        try {
            const { execSync } = require('child_process');
            const wmOut = execSync('wmctrl -lG', { 
        ...
        // Get all windows with PIDs
        try {
            const { execSync } = require('child_process');
            const wmPid = execSync('wmctrl -lp', { 
                env: { ...process.env, DISPLAY: this.display },
                timeout: 2000
            }).toString().trim();
        ...
        // Search for browser windows by class
        try {
            const { execSync } = require('child_process');
            const xd = execSync(`xdotool search --class ${this.browserManager.className} || true`, { 
                env: { ...process.env, DISPLAY: this.display },
                timeout: 2000
            }).toString().trim();
            this.logger.debug(`xdotool search --class ${this.browserManager.className} output: ${xd || '(none)'}`);
        } catch (e) {
        ...
        try {
            const { execSync } = require('child_process');
            const active = execSync('xdotool getactivewindow', { 
                env: { ...process.env, DISPLAY: this.display },
                timeout: 2000
            }).toString().trim();
            this.logger.debug(`Active window: ${active}`);
        } catch (e) {
            this.logger.debug('Failed to get active window: ' + e.message);
        }

        this.logger.debug('=== End Diagnostics ===');
    }

    /**
     * Helper: Find MPV window with retry logic
     * @param {number} maxRetries - Maximum number of retry attempts (default: 3)
     * @param {number} retryDelay - Delay between retries in ms (default: 1000)
     * @returns {string|null} MPV window ID or null if not found
     */
    async _findMpvWindow(maxRetries = 3, retryDelay = 1000) {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const mpvWindow = this.windowManager.getWindowIdByNameExact('ParadoxMPV');
                if (mpvWindow) {
                    this.logger.debug(`Found MPV window by name (attempt ${attempt}): ${mpvWindow}`);
                    return mpvWindow;
                }
            } catch (err) {
                this.logger.debug(`getWindowIdByNameExact attempt ${attempt} failed: ${err.message}`);
            }

            // Try class-based search as fallback
            try {
                const mpvWindows = await this.windowManager.searchByClass('mpv');
                if (mpvWindows && mpvWindows.length > 0) {
                    const mpvWindow = mpvWindows[0];
                    this.logger.debug(`Found MPV window by class (attempt ${attempt}): ${mpvWindow}`);
                    return mpvWindow;
                }
            } catch (err) {
                this.logger.debug(`searchByClass('mpv') attempt ${attempt} failed: ${err.message}`);
            }

            // If not last attempt, wait before retrying
            if (attempt < maxRetries) {
                this.logger.debug(`MPV window not found, waiting ${retryDelay}ms before retry ${attempt + 1}/${maxRetries}`);
                await new Promise(r => setTimeout(r, retryDelay));
            }
        }

        this.logger.warn(`MPV window not found after ${maxRetries} attempts`);
        return null;
    }
    ...
    async _hideBrowser() {
        ...
        this.logger.info('Hiding browser - returning focus to MPV');

        // Find MPV window with retry logic
        const mpvWindow = await this._findMpvWindow(3, 1000);
        
        if (!mpvWindow) {
            this.logger.error('MPV window not found - cannot hide browser properly');
            // Still update state even if MPV not found
            this.currentState.focus = 'mpv';
            this.currentState.content = this.currentState.currentVideo || this.currentState.currentImage;
            this.publishStatus();
            this.publishEvent({
        ...
        // Try multiple activation methods with retry
        let activated = false;
        
        // Attempt 1: Standard xdotool activation
        try {
            this.windowManager.activateWindow(mpvWindow);
            await new Promise(r => setTimeout(r, 100));
            activated = this.windowManager.isWindowActive(mpvWindow);
            if (activated) {
                this.logger.debug('MPV activated successfully with xdotool');
            }
        } catch (err) {
            this.logger.debug(`xdotool activation failed: ${err.message}`);
        }

        // Attempt 2: wmctrl fallback
        if (!activated) {
            try {
                const success = await this.windowManager.wmctrlActivate(mpvWindow);
                if (success) {
                    await new Promise(r => setTimeout(r, 100));
                    activated = this.windowManager.isWindowActive(mpvWindow);
                    if (activated) {
                        this.logger.debug('MPV activated successfully with wmctrl');
                    }
                }
            } catch (err) {
                this.logger.debug(`wmctrl activation failed: ${err.message}`);
            }
        }
        ...
        if (!activated) {
            try {
                ...
                activated = this.windowManager.isWindowActive(mpvWindow);
                if (activated) {
                    this.logger.info('MPV activated successfully with aggressive fallback');
                }
            } catch (err) {
                this.logger.debug(`Aggressive fallback failed: ${err.message}`);
            }
        }

        if (!activated) {
            this.logger.warn('Failed to activate MPV window after all attempts');
        }
