...
class AudioZone extends BaseZone {
    ...
    async handleCommand(command) {
        ...
        try {
            switch (command.command) {
                case 'playBackground': {
                    const ok = await this._playBackgroundMusic(command.file || command.audio, command.volume, command.loop);
    ...
    async _playSpeech(audioPath, volume, ducking) {
        ...
        // Only treat configured defaults as valid if they are negative (ducking reduces volume)
        const zoneDefault = (this.audioConfig.duckingVolume !== null && this.audioConfig.duckingVolume < 0)
            ? this.audioConfig.duckingVolume
            : undefined;
        const globalDefault = (this.config && this.config.speechDucking !== undefined && this.config.speechDucking < 0)
            ? this.config.speechDucking
            : undefined;
