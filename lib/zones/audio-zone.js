...
class AudioZone extends BaseZone {
    constructor(config, mqttClient, zoneManager) {
        ...
        // Debug logging for dual output mode
        if (this.audioConfig.dualOutputMode) {
            this.logger.info(`Dual output mode enabled for zone ${this.audioConfig.zoneId}`);
            this.logger.info(`Primary device: ${this.audioConfig.primaryDevice}`);
            this.logger.info(`Secondary device: ${this.audioConfig.secondaryDevice}`);
        }
    ...
    async handleCommand(command) {
        ...
        try {
            ...
            // If command was a system control, publish success immediately
            ...
            switch (command.command) {
                ...
                // Speech commands
                case 'playSpeech':
                    await this._playSpeech(command.audio || command.file, command.volume, command.ducking);
                    break;
                case 'pauseSpeech':
                ...
                case 'sleepScreen':
                case 'wakeScreen':
                    // Audio zones ignore screen sleep/wake commands
                    this.logger.info(`Ignoring ${command.command} command: audio zones do not control screens`);
                    this.publishEvent({ 
                        [command.command.replace('Screen', '_screen')]: false, 
                        ignored: true, 
                        reason: 'audio_zone' 
                    });
                    break;
    ...
    getSupportedCommands() {
        return [
            'playBackground', 
            'pauseBackground', 'resumeBackground', 'stopBackground',
    ...
    async _pauseBackgroundMusic() {
        ...
        if (!this.currentState.backgroundMusic.playing) {
            this.logger.warn('No background music playing to pause');
            return;
        }
    ...
    async _resumeBackgroundMusic() {
        ...
        if (!this.currentState.backgroundMusic.playing) {
            this.logger.warn('No background music to resume');
            return;
        }
    ...
    async _playSpeech(audioPath, volume, ducking) {
        ...
        const result = await this.audioManager.playSpeech(fullPath, {
            volume: targetVolume,
            duckVolume: this.audioConfig.duckingVolume // This is for legacy compatibility
        });
        
        if (!result.success) {
            // Remove ducking on failure
            if (duckingLevel > 0) {
        ...
        // Note: In a real implementation, this should be based on actual speech completion events
        // For now, using a timeout as a simplified approach
        if (duckingLevel > 0) {
            setTimeout(() => {
    ...
    async _pauseSpeech() {
        ...
        this.mpvInstances.speech.status = 'paused';

        this.publishStatus();
        this.publishEvent({ speech_paused: true });
    }
    ...
    async _resumeSpeech() {
        ...
        this.publishStatus();
        this.publishEvent({ speech_resumed: true });
    }
    ...
    async _skipSpeech() {
        ...
        this.publishStatus();
        this.publishEvent({ speech_skipped: true });
    }
    ...
    async _pauseAudio() {
        this.logger.info('Pausing all audio');
        ...
        if (this.mpvInstances.speech.status === 'playing') {
            await this._pauseSpeech();
        }

        this.publishEvent({ all_audio_paused: true });
    }

    async _resumeAudio() {
        this.logger.info('Resuming all audio');

        // Resume background music if paused
        if (this.mpvInstances.background.status === 'paused') {
            await this._resumeBackgroundMusic();
        }

        // Resume speech if paused
        if (this.mpvInstances.speech.status === 'paused') {
            await this._resumeSpeech();
        }

        this.publishEvent({ all_audio_resumed: true });
    }

    async _stopAudio() {
        // Alias for _stopAll for consistency
        await this._stopAll();
    }
    ...
    _getCurrentBackgroundVolume() {
        return this.currentState.backgroundMusic.volume || this.audioConfig.defaultVolume;
    }
