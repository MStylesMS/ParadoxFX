...
class AudioDevice {
    constructor(config, mqttClient) {
        ...
        const audioConfig = {
            ...
            ipcSocketPath: config.ipcSocketPath || `/tmp/mpv-audio-${require('../utils/utils').sanitizeFilename(config.name)}-socket`
        };
    ...
    async _handlePlayBackgroundMusic(params) {
        ...
        let validatedVolume = volume;
        if (validatedVolume !== undefined) {
            if (typeof validatedVolume !== 'number' || validatedVolume < 0 || validatedVolume > 150) {
                throw new Error('Volume must be between 0 and 150');
            }
        } else {
            validatedVolume = this.status.backgroundMusic.volume;
        }
        this.logger.info(`Playing background music: ${file}`);
        // Construct full file path based on media directory
        const mediaPath = this.config.mediaPath || '/opt/paradox/media';
        const mediaDir = this.config.media_dir || 'background';
        const fullPath = path.join(mediaPath, mediaDir, file);
        await this.audioManager.playBackgroundMusic(fullPath, validatedVolume);
    ...
    async _handlePlaySpeech(params) {
        const { file, duckVolume = 30, volume } = params;
        if (!file) {
            throw new Error('Speech file parameter required');
        }
        let validatedDuckVolume = duckVolume;
        if (typeof validatedDuckVolume !== 'number' || validatedDuckVolume < 0 || validatedDuckVolume > 150) {
            throw new Error('Ducking volume must be between 0 and 150');
        }
        let validatedVolume = volume;
        if (validatedVolume !== undefined) {
            if (typeof validatedVolume !== 'number' || validatedVolume < 0 || validatedVolume > 150) {
        ...
        this.logger.info(`Playing speech: ${file} (duck to ${validatedDuckVolume}%)`);
        ...
        await this.audioManager.playSpeech(file, { duckVolume: validatedDuckVolume, volume: validatedVolume });
